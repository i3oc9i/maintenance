...
| `--section <name,...>` | Run only named section(s) without prompts; comma-separated. Names: `brew-formulae`, `brew-casks`, `rust`, `mise`, `uv`, `pre-commit`. Alias: `brew` = `brew-formulae,brew-casks` |
...
- **Section functions**: Each section (`do_brew_formulae`, `do_brew_casks`, `do_rust`, `do_mise`, `do_uv`, `do_pre_commit`) is a standalone function dispatched through `run_section`.
...
- **uv tools**: Runs `uv tool upgrade --all` to upgrade every package installed via `uv tool install`.
